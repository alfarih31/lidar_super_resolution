...
# Global Variables
image_rows_low = 64 # 8, 16, 32
image_rows_high = 64 # 16, 32, 64
image_cols_low = 512
image_cols_high = 1024
channel_num = 1

upscaling_factor = int(image_rows_high / image_rows_low)
print('#'*50)
print('Input image resolution:   {} by {}'.format(image_rows_low, image_cols_low))
print('Output image resolution:  {} by {}'.format(image_rows_high, image_cols_high))
...
# ouster
ang_res_x = 360.0/float(image_cols_high) # horizontal resolution
...
def get_low_res_from_high_res(high_res_data):
    mask =  [j for j in range(0, image_cols_high, int(image_cols_high/image_cols_low))]
    low_res_data = high_res_data[:, :, mask]
    return low_res_data
